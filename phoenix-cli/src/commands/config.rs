...
pub fn run(action: ConfigAction) -> Result<()> {
    match action {
        ConfigAction::Init { soc, name, output } => init(&soc, &name, output.as_deref()),
...
pub fn init(soc: &str, name: &str, output: Option<&str>) -> Result<()> {
    ...
    println!(
        "  3. Run: phoenix generate dts --board {} --output device.dts",
        output_path
    );
...
pub fn validate(path: &str) -> Result<()> {
    ...
    println!(
        "Memory: {} MB {}",
        config.hardware.memory.size_mb, config.hardware.memory.mem_type
    );
    println!(
        "Storage: {} GB {}",
        config.hardware.storage.size_gb, config.hardware.storage.storage_type
    );
