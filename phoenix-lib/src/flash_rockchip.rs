...
impl RkImageHeader {
    ...
    fn parse_rkaf(buf: &[u8]) -> Result<Self, AppError> {
        ...
        for i in 0..count {
            ...
            let name = String::from_utf8_lossy(&buf[p..p + 0x20])
                .trim_matches('\0')
                .to_string();
    ...
    fn parse_rkfw(buf: &[u8]) -> Result<Self, AppError> {
        ...
        let chip_family = match buf[0x15] {
            0x50 => "rk29xx",
            0x60 => "rk30xx",
            0x70 => "rk31xx",
            0x80 => "rk32xx",
...
impl RockchipDevice {
    ...
    pub fn flash_update_image(
        ...
        let total = header.entries.len();

        for (i, entry) in header.entries.iter().enumerate() {
            if entry.path == "SELF" {
                continue;
            }
            info!("Flashing entry: {} ({} bytes)", entry.name, entry.file_size);
