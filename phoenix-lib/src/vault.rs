...
impl VaultManager {
    ...
    pub fn create_vault(
        ...
        if vault_path.exists() {
            return Err(AppError::ValidationError(format!("Vault '{}' already exists", name)));
        }
        
        fs::create_dir_all(&vault_path)
            .map_err(|e| AppError::IoError(e.to_string()))?;
            
        info!("Creating vault '{}' for device {}", name, report.variant_id.as_deref().unwrap_or("unknown"));
        ...
        let manifest = VaultManifest {
            ...
            soc: report.usb_devices.get(0).map(|d| d.soc_family.clone()).unwrap_or_default(),
            partitions,
    ...
    pub fn verify_vault(&self, name: &str) -> Result<bool, AppError> {
        let vault_path = self.base_dir.join(name);
        if !vault_path.exists() {
            return Err(AppError::DeviceNotFound(format!("Vault '{}' not found", name)));
        }
    ...
    pub fn extract_from_vault(
        ...
        info!("Extracting data from partition {} of vault {} to {}", partition, name, output_file.display());
        
        // Simulate extracting NVRAM
        let dummy_nvram = "mac_addr=00:11:22:33:44:55\nwifi_calib=ff:00:11:22\n";
        fs::write(output_file, dummy_nvram)
            .map_err(|e| AppError::IoError(e.to_string()))?;
