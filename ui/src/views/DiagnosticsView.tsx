        ...
        <div class="h-full flex flex-col gap-6 font-mono">
            <header class="flex flex-col gap-1">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-2xl font-black tracking-tighter text-text-primary uppercase italic">Archaeological Forensics</h2>
                </div>
                <p class="text-[10px] text-text-muted uppercase tracking-[0.3em] pl-5 italic">Deep Hardware Probing | Layer 1 — Hardware Liberation</p>
            </header>
            ...
            <div class="grid lg:grid-cols-12 gap-6 flex-1 min-h-0">
                ...
                <div class="lg:col-span-3 flex flex-col gap-2">
                    <Button
                        ...
                        class={cn(
                            "h-12 w-full justify-start px-6 rounded-none border-none text-[10px] font-black uppercase tracking-widest italic",
                    ...
                    <div class="mt-auto border border-dashed border-border-subtle p-5 space-y-4 bg-sidebar/20 rounded-sm italic font-bold">
                        ...
                        <Button
                            ...
                            isLoading={loading()}
                            glow
                            class="w-full h-11 text-[9px] font-black rounded-none transition-ui tracking-widest italic"
                        >
                            INITIATE DEEP PROBE
                        </Button>
                    </div>
                        ...
                        <Match when={activeTab() === 'hardware'}>
                            <div class="grid md:grid-cols-2 gap-6 overflow-y-auto custom-scrollbar pr-2 pb-4">
                                <Card glow="accent" title="System on Chip" subtitle="Silicon ID & register trace">
                                    <Show when={report()?.usbDevices?.[0]} fallback={<div class="py-12 text-center text-[9px] text-text-muted font-black uppercase italic opacity-30">Awaiting Silicon Probe...</div>}>
                                        <PropertyGrid>
                                ...
                                <Card glow="accent" title="Memory Latency" subtitle="SDRAM timings & vendor match">
                                    <Show when={report()?.ddrTiming} fallback={<div class="py-12 text-center text-[9px] text-text-muted font-black uppercase italic opacity-30">Probe required...</div>}>
                                        <PropertyGrid>
                                        ...
                                        <div class="mt-4 pt-4 border-t border-border-subtle bg-white/[0.01] p-3 rounded-sm">
                                            <span class="text-[8px] font-black text-text-muted uppercase italic opacity-60 block mb-3">Compatible Device Trees:</span>
                                ...
                                <Card glow="accent" title="Bootloader State" subtitle="Secure boot & signature verification">
                                    <Show when={report()?.bootloader} fallback={<div class="py-12 text-center text-[9px] text-text-muted font-black uppercase italic opacity-30">Awaiting stage analysis...</div>}>
                                        <PropertyGrid>
                                        ...
                                        <Show when={report()?.bootloader?.notes?.length}>
                                            <div class="mt-4 border-t border-border-subtle pt-4 space-y-2">
                                                <For each={report()?.bootloader?.notes}>
                                                    {(note) => <p class="text-[9px] text-accent font-black italic uppercase tracking-tight pl-2 border-l-2 border-accent/20">Trace: {note}</p>}
                                                </For>
                                            </div>
                                        </Show>
                                    </Show>
                                ...
                                <Card glow="accent" title="IO Matrix" subtitle="Storage & Wireless controllers">
                                    <div class="space-y-6">
                                        <Show when={report()?.wifiChip} fallback={<div class="text-[9px] text-text-muted font-black uppercase italic opacity-30">Wireless node masked...</div>}>
                                            <div class="space-y-1">
                                                <span class="text-[8px] font-black text-text-muted uppercase italic opacity-60 block">Wireless Controller</span>
                                                <PropertyGrid>
                                        ...
                                        <Show when={report()?.emmcInfo} fallback={<div class="text-[9px] text-text-muted font-black uppercase italic border-t border-border-subtle pt-4 opacity-30">Storage node masked...</div>}>
                                            <div class="space-y-1 pt-4 border-t border-border-subtle">
                                                <span class="text-[8px] font-black text-text-muted uppercase italic opacity-60 block">Flash Storage</span>
                                                <PropertyGrid>
                            ...
                            <div class="grid gap-6 overflow-y-auto custom-scrollbar pr-2 pb-4">
                                <Card glow="rose" title="Threat Detection Vault" subtitle="Scanning for malicious OEM telemetry & backdoors">
                                    <Show when={securityReport()} fallback={
                                        <div class="flex flex-col items-center py-24 grayscale opacity-20 select-none">
                                            ...
                                            <span class="text-[10px] tracking-[0.5em] font-black uppercase italic">Vault Analysis Required</span>
                                            <p class="text-[9px] text-text-muted mt-2 uppercase tracking-widest italic font-bold">Mount firmware blob to initiate forensics</p>
                                        </div>
                                    }>
                                        <Show when={securityReport()?.isInfected} fallback={
                                            <div class="flex flex-col items-center py-14 bg-emerald-500/5 border border-emerald-500/10 italic rounded-sm">
                                                <div class="w-3 h-3 rounded-full bg-emerald-500 animate-pulse mb-4 shadow-glow shadow-emerald-500/40" />
                                                <h3 class="text-xs font-black text-emerald-400 uppercase tracking-widest italic leading-none mb-1">Signal Integrity Green</h3>
                                                <p class="text-[9px] text-text-muted mt-2 uppercase italic font-bold opacity-60">No known malware signatures found</p>
                                            </div>
                                        }>
                                            <div class="space-y-4">
                                                <For each={securityReport()?.threats}>
                                                    {(threat) => (
                                                        <div class="p-4 border border-rose-500/20 bg-rose-500/5 space-y-3 italic group hover:bg-rose-500/10 transition-ui">
                                                            <div class="flex justify-between items-center">
                                                                <span class="text-[10px] font-black text-rose-500 uppercase tracking-widest">{threat.name}</span>
                                                                <Badge variant={getSeverityColor(threat.severity)} class="px-3 font-black tracking-widest italic">{threat.severity}</Badge>
                                                            </div>
                                ...
                                <div class="grid sm:grid-cols-2 gap-6">
                                    <Card glow="rose" title="Known OEM Malware" subtitle="Common TV Box threats">
                                        <div class="space-y-4">
                                            <div class="p-4 bg-sidebar/50 border border-border-subtle hover:border-rose-500/30 transition-ui cursor-default italic rounded-sm group">
                                                <div class="flex items-center gap-2 mb-2">
                                                    <div class="w-1.5 h-1.5 bg-rose-500 group-hover:animate-pulse" />
                                                    <span class="text-[10px] font-black text-text-primary uppercase tracking-widest italic">Corejava Botnet</span>
                                                </div>
                                            ...
                                            <div class="p-4 bg-sidebar/50 border border-border-subtle hover:border-orange-500/30 transition-ui cursor-default italic rounded-sm group">
                                                <div class="flex items-center gap-2 mb-2">
                                                    <div class="w-1.5 h-1.5 bg-orange-500 group-hover:animate-pulse" />
                                                    <span class="text-[10px] font-black text-text-primary uppercase tracking-widest italic">BadBox Proxy</span>
                                                </div>
                                    ...
                                    <Card glow="accent" title="Forensic Techniques" subtitle="Register-level analysis">
                                        <div class="space-y-4 italic text-[10px] text-text-muted leading-relaxed font-bold uppercase py-1">
                                            <p><span class="text-accent font-black tracking-widest underline decoration-accent/20 underline-offset-4 mr-2 italic">HEAP SCAN:</span> Analyzing SRAM for residue OEM keys.</p>
                                            <p><span class="text-accent font-black tracking-widest underline decoration-accent/20 underline-offset-4 mr-2 italic">GIFT WRAP:</span> Identifying masked partitions in EMMC controller.</p>
                                            <p><span class="text-accent font-black tracking-widest underline decoration-accent/20 underline-offset-4 mr-2 italic">DTB DUMP:</span> Reconstructing hardware tree from binary blobs.</p>
                                        </div>
                                    </Card>
                                </div>
                            </div>
                            ...
                            <div class="space-y-6 overflow-y-auto custom-scrollbar pr-2 pb-4">
                                <div class="grid sm:grid-cols-3 gap-6">
                                    <Card glow="accent" class="text-center py-10">
                                        <span class="text-[9px] font-black text-text-muted uppercase tracking-[0.3em] mb-4 block italic opacity-60">Memory Profile</span>
                                        <span class="text-3xl font-black text-accent italic tracking-tighter drop-shadow-sm uppercase">Low RAM</span>
                                        <p class="text-[9px] text-text-muted uppercase mt-3 italic font-bold">Target: 1GB LPDDR3</p>
                                    </Card>
                                    <Card glow="accent" class="text-center py-10">
                                        <span class="text-[9px] font-black text-text-muted uppercase tracking-[0.3em] mb-4 block italic opacity-60">Compcache Status</span>
                                        <span class="text-3xl font-black text-accent italic tracking-tighter drop-shadow-sm uppercase">512 MB</span>
                                        <p class="text-[9px] text-text-muted uppercase mt-3 italic font-bold">ZRAM Node Active</p>
                                    </Card>
                                    <Card glow="accent" class="text-center py-10">
                                        <span class="text-[9px] font-black text-text-muted uppercase tracking-[0.3em] mb-4 block italic opacity-60">Power Governor</span>
                                        <span class="text-3xl font-black text-accent italic tracking-tighter drop-shadow-sm uppercase">Balanced</span>
                                        <p class="text-[9px] text-text-muted uppercase mt-3 italic font-bold">Mode: Interactive</p>
                                    </Card>
                                </div>
                                ...
                                <Card glow="accent" title="Optimization Matrix" subtitle="Proposed system-level tweaks">
                                    <div class="space-y-3">
                                        <For each={[
                                            ...
                                            {(prop) => (
                                                <div class="flex items-center justify-between p-5 bg-sidebar/50 border border-border-subtle group hover:border-accent/30 transition-ui italic rounded-sm">
                                                    <div class="space-y-1.5">
                                                        <code class="text-[11px] font-black text-accent uppercase tracking-tight italic">{prop.key}={prop.value}</code>
                                                        <p class="text-[9px] text-text-muted font-bold uppercase opacity-60 tracking-wider leading-none italic">{prop.desc}</p>
                                                    </div>
